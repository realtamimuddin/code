...
function createYjsClient(options) {
  const doc2 = new Doc();
  ...
  const provider = new WebsocketProvider(urlWithRoom, roomId, doc2, { connect: true });
  const highlightsByFile = doc2.getMap("highlights");
  const addHighlight = (rec) => {
    const fileMap = highlightsByFile.get(rec.filePath) ?? new YMap();
  ...
  const removeHighlight = (filePath, id2) => {
  ...
  const subscribe2 = (cb) => {
      ...
      highlightsByFile.forEach((fileMap, filePath) => {
        fileMap.forEach((yrec) => {
          const rec = {
            ...
            userId: yrec.get("userId") ?? void 0,
            avatarUrl: yrec.get("avatarUrl") ?? void 0,
    ...
    doc2.on("update", handler);
    handler();
    return () => {
      doc2.off("update", handler);
    };
  };
  return { doc: doc2, provider, highlightsByFile, addHighlight, removeHighlight, subscribe: subscribe2 };
}
function generateHighlightId(filePath, startLine, endLine) {
  return `${filePath}:${startLine}-${endLine}:${Math.random().toString(36).slice(2, 8)}`;
}

// src/content/githubOverlay.ts
var ACTIVE_HIGHLIGHTS_ATTR = "data-prh-highlight";
function isPrFilesPage() {
  return /\/pull\/\d+\/files/.test(location.pathname);
}
function getFilesRoot() {
  const el = document.querySelector("#files") || document.querySelector("[aria-label='Files changed']");
  return el;
}
function getFilePathFromHeader(fileContainer) {
  const headerLink = fileContainer.querySelector(".file-info a.Link--primary, .file-info a.text-mono");
  if (headerLink?.textContent) return headerLink.textContent.trim();
  const header = fileContainer.querySelector("[data-path]");
  if (header) return header.getAttribute("data-path") || "unknown";
  return "unknown";
}
function collectLineNumberCells(fileContainer) {
  const cells = Array.from(fileContainer.querySelectorAll("td.blob-num[data-line-number]"));
  return cells;
}
function computeCodeArea(fileContainer) {
  const firstCodeCell = fileContainer.querySelector("td.blob-code");
...
function ensureOverlay(fileContainer) {
...
function findLineFromPoint(ctx, clientY) {
...
function renderHighlights(ctx, records) {
  ...
  const lineToTop = /* @__PURE__ */ new Map();
  for (const cell of ctx.lineNumberCells) {
    const ln = parseInt(cell.getAttribute("data-line-number") || "0", 10);
    const rect = cell.getBoundingClientRect();
    const parentRect = ctx.container.getBoundingClientRect();
...
function setupInteractions(ctx, add, getUserColor, getUserId) {
  let dragging = false;
  let startLine = null;
...
function init() {
  ...
  if (!filesRoot) return;
  const { subscribe: subscribe2, addHighlight } = createYjsClient();
  const userColor = () => "rgba(255, 235, 59, 0.35)";
  const userId = () => void 0;
  const fileContexts = [];
  const setupForContainer = (c) => {
    const ctx = ensureOverlay(c);
    fileContexts.push(ctx);
    setupInteractions(ctx, addHighlight, userColor, userId);
  };
  const containers = Array.from(filesRoot.querySelectorAll(".js-file"));
  ...
  const mo = new MutationObserver((muts) => {
    for (const m of muts) {
      m.addedNodes.forEach((n) => {
        ...
        n.querySelectorAll?.(".js-file").forEach((nn) => setupForContainer(nn));
      });
    }
  });
  mo.observe(filesRoot, { childList: true, subtree: true });
  let lastRecords = [];
  const rerender = (records) => {
    lastRecords = records;
    for (const ctx of fileContexts) {
      renderHighlights(ctx, records);
    }
  };
  const unsubscribe2 = subscribe2(rerender);
  const onScrollOrResize = () => {
    for (const ctx of fileContexts) {
      const codeArea = function computeCodeArea2(fileContainer) {
        const firstCodeCell = fileContainer.querySelector("td.blob-code");
        if (!firstCodeCell) return { left: 0, width: fileContainer.clientWidth };
        const rect = firstCodeCell.getBoundingClientRect();
        const parentRect = fileContainer.getBoundingClientRect();
